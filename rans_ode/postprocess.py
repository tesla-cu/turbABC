...
from pyabc.utils import timer, pdf_from_array_with_x
...
def main():
    ...
    # ####################################################################################################################
    logging.info('\n#############Calibration############')
    calibration = np.load(os.path.join(path['calibration'], 'calibration.npz'))['C']
    calibration_dist = np.load(os.path.join(path['calibration'], 'calibration.npz'))['dist']
    C_limits = np.loadtxt(os.path.join(path['calibration'], 'C_limits'))
    # eps_k = plotting.plot_dist_pdf(path, dist, g.x)
    # # np.savetxt(os.path.join(path['output'], 'eps.txt'), [eps_k])
