...
[Tlrr,Ylrr] = ode45(@lrr,[0:dt:tmax],[a110,a220,a330,0,0,0,1,1]); %LRR RST model
...
subplot('Position',[x,y,w,h]);plot(decay2(:,1),2*decay2(:,2),'ok','MarkerSize',msize)
%subplot('Position',[x,y,w,h]);plot(decay2_lrr(:,1),2*decay2_lrr(:,2),'dk','MarkerSize',msize)
hold off;
set(gca,'FontSize',pfont,'FontName','Times')
text(0.08,0.29,'$a_{11}$','Interpreter','Latex','FontSize',tfont) 
text(0.08,-0.05,'$a_{22}$','Interpreter','Latex','FontSize',tfont) 
text(0.08,-0.21,'$a_{33}$','Interpreter','Latex','FontSize',tfont) 
...
xs=0.3; %LEFT edge of legend
ys=-0.26; %TOP edge of legend
...
line([xs+(i-1)*xsp+xmark,xs+(i-1)*xsp+xmark],[(ys-(j-1)*ysp),(ys-(j-1)*ysp)],...
    'Marker','o','Color','k','MarkerSize',msize)
