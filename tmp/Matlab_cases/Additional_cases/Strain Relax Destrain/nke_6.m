...
S11=a4*(t-t6); %applied strain
S11e=a1*(lam*exp(-(t-t1)/lam)-(lam+t1-t2)*exp(-(t-t2)/lam))...
    +a2*(lam*exp(-(t-t3)/lam)-(lam-t2+t3)*exp(-(t-t2)/lam))...
    -a3*(lam*exp(-(t-t4)/lam)-(lam+t4-t5)*exp(-(t-t5)/lam))...
    +a4*(-lam+t-t6+(lam-t5+t6)*exp(-(t-t5)/lam));     %effective strain
