...
global k0e0 a1 a2 a3 a4 S0 Lt t1 t2 t3 t4 t5 t6 cmu c ce1 ce2 tr a b B0 B1 B2 B3 B4
...
terf=[0:dt:t4];
Serf=10*(0.5+0.5*erf(10*(terf-0.3)));

a=9.5;
b=0.008;
Sgauss=a*exp(-(terf-t2).^2./b);
...
subplot('Position',[x,y,w,h]);plot(terf,Sgauss,'-k','LineWidth',lwid)
...
%Solve NKE ODEs
[T1nke,Y1nke] = ode45(@nke_1,[0:dt:t1],[1,1]);
lk=length(Y1nke(:,1));
le=length(Y1nke(:,2));
[T2nke,Y2nke] = ode45(@nke_2,[t1+dt:dt:t2],[Y1nke(lk,1),Y1nke(le,2)]);
lk=length(Y2nke(:,1));
le=length(Y2nke(:,2));
[T3nke,Y3nke] = ode45(@nke_3,[t2+dt:dt:t3],[Y2nke(lk,1),Y2nke(le,2)]);
lk=length(Y3nke(:,1));
le=length(Y3nke(:,2));
[T4nke,Y4nke] = ode45(@nke_4,[t3+dt:dt:t4],[Y3nke(lk,1),Y3nke(le,2)]);
lk=length(Y4nke(:,1));
le=length(Y4nke(:,2));
[T5nke,Y5nke] = ode45(@nke_5,[t4+dt:dt:t5],[Y4nke(lk,1),Y4nke(le,2)]);
lk=length(Y5nke(:,1));
le=length(Y5nke(:,2));
[T6nke,Y6nke] = ode45(@nke_6,[t5+dt:dt:t6],[Y5nke(lk,1),Y5nke(le,2)]);
...
%Solve time-local NKE ODEs
[T1nket,Y1nket] = ode45(@nke_t1,[0:dt:t1],[1,1]);
lk=length(Y1nket(:,1));
le=length(Y1nket(:,2));
[T2nket,Y2nket] = ode45(@nke_t2,[t1+dt:dt:t2],[Y1nket(lk,1),Y1nket(le,2)]);
lk=length(Y2nket(:,1));
le=length(Y2nket(:,2));
[T3nket,Y3nket] = ode45(@nke_t3,[t2+dt:dt:t3],[Y2nket(lk,1),Y2nket(le,2)]);
lk=length(Y3nket(:,1));
le=length(Y3nket(:,2));
[T4nket,Y4nket] = ode45(@nke_t4,[t3+dt:dt:t4],[Y3nket(lk,1),Y3nket(le,2)]);
lk=length(Y4nket(:,1));
le=length(Y4nket(:,2));
[T5nket,Y5nket] = ode45(@nke_t5,[t4+dt:dt:t5],[Y4nket(lk,1),Y4nket(le,2)]);
lk=length(Y5nket(:,1));
le=length(Y5nket(:,2));
[T6nket,Y6nket] = ode45(@nke_t6,[t5+dt:dt:t6],[Y5nket(lk,1),Y5nket(le,2)]);

[Tnkeg,Ynkeg] = ode113(@nke_g,[0:dt:t4],[1,1]);
...
%88888888888888888888888888888888888888888888888888888888888888888888888888
%Find time-local NKE anisotropy time series
ke=Y1nket(:,1)./Y1nket(:,2);
lam=c*ke; %memory time scale
S11=ke-ke; %strain
S11e=ke-ke; %effective strain
a1nket=-2*cmu*ke.*S11e; %anisotropy 

ke=Y2nket(:,1)./Y2nket(:,2);
lam=c*ke; %memory time scale
S11=a1*(T2nket-t1); %strain
S11e=S11-tr*lam.*a1; %effective strain
a2nket=-2*cmu*ke.*S11e; %anisotropy 

ke=Y3nket(:,1)./Y3nket(:,2);
lam=c*ke; %memory time scale
S11=-a2*(T3nket-t3); %strain
S11e=S11-tr*lam.*(-a2);  %effective strain
a3nket=-2*cmu*ke.*S11e; %anisotropy 

ke=Y4nket(:,1)./Y4nket(:,2);
lam=c*ke; %memory time scale
S11=ke-ke; %strain
S11e=S11;  %effective strain
a4nket=-2*cmu*ke.*S11e; %anisotropy 

ke=Y5nket(:,1)./Y5nket(:,2);
lam=c*ke; %memory time scale
S11=-a3*(T5nket-t4); %strain
S11e=S11-tr*lam.*(-a3);   %effective strain
a5nket=-2*cmu*ke.*S11e; %anisotropy 

ke=Y6nket(:,1)./Y6nket(:,2);
lam=c*ke; %memory time scale
S11=a4*(T6nket-t6); %strain
S11e=S11-tr*lam.*a4;  %effective strain
a6nket=-2*cmu*ke.*S11e; %anisotropy 
%88888888888888888888888888888888888888888888888888888888888888888888888888
...
%subplot('Position',[x,y,w,h]);plot(T1nke,sc2*a1nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T2nke,sc2*a2nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T3nke,sc2*a3nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T4nke,sc2*a4nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T5nke,sc2*a5nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T6nke,sc2*a6nke,'-b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T1nket,sc2*a1nket,'--b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T2nket,sc2*a2nket,'--b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T3nket,sc2*a3nket,'--b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T4nket,sc2*a4nket,'--b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T5nket,sc2*a5nket,'--b','LineWidth',lwid)
%subplot('Position',[x,y,w,h]);plot(T6nket,sc2*a6nket,'--b','LineWidth',lwid)
subplot('Position',[x,y,w,h]);plot(Tnkeg,sc2*ankeg,'-b','LineWidth',lwid)
subplot('Position',[x,y,w,h]);plot(Tnkeg1,sc2*ankeg1,'-b','LineWidth',1)
subplot('Position',[x,y,w,h]);plot(Tnkeg2,sc2*ankeg2,':b','LineWidth',lwid)
subplot('Position',[x,y,w,h]);plot(Tnkeg3,sc2*ankeg3,'--b','LineWidth',lwid)
subplot('Position',[x,y,w,h]);plot(Tnkeg4,sc2*ankeg4,'-.b','LineWidth',lwid)
hold off;
%annotation('doublearrow',[0.35,0.57],[0.60,0.60],'LineWidth',1,'Color','k')
%annotation('doublearrow',[0.57,0.688],[0.33-yoff,0.33-yoff],'LineWidth',1,'Color','k')
%annotation('doublearrow',[0.688,0.88],[0.26-yoff,0.26-yoff],'LineWidth',1,'Color','k')
%text(0.32,0.21*sc2+yoff3,'Straining','FontSize',tfont,'Interpreter','Latex')
%text(0.57,-0.82*sc2-yoff2,'Relax.','FontSize',tfont,'Interpreter','Latex')
%text(0.72,-1.02*sc2-yoff2,'Destraining','FontSize',tfont,'Interpreter','Latex')
set(gca,'FontSize',pfont,'FontName','Times')
%set(gca,'YTick',[-1,-0.5,0,0.5,1])
ylabel('$a_{11}$','Interpreter','latex','Rotation',0,...
    'FontSize',yfont,'Position',[xlow-(xhigh-xlow)/yx,(ylow+yhigh)/2])
xlabel('$t\epsilon_0/k_0$','Interpreter','latex','FontSize',xfont,...
    'Position',[(xlow+xhigh)/2,-1.56*sc2+0.32])
...
text(xs+(i-1)*xsp+len+xlsp,(ys-(j-1)*ysp),'NKE, N=1','Interpreter',...
    'latex','FontSize',legfont)
...
text(xs+(i-1)*xsp+len+xlsp,(ys-(j-1)*ysp),'NKE, N=2','Interpreter',...
    'latex','FontSize',legfont)
...
line([xs+(i-1)*xsp,xs+(i-1)*xsp+len],[(ys-(j-1)*ysp),(ys-(j-1)*ysp)],...
    'LineStyle','--','Color','b','LineWidth',lwid)
text(xs+(i-1)*xsp+len+xlsp,(ys-(j-1)*ysp),'NKE, N=3','Interpreter',...
    'latex','FontSize',legfont)
...
line([xs+(i-1)*xsp,xs+(i-1)*xsp+len],[(ys-(j-1)*ysp),(ys-(j-1)*ysp)],...
    'LineStyle','-.','Color','b','LineWidth',lwid)
text(xs+(i-1)*xsp+len+xlsp,(ys-(j-1)*ysp),'NKE, N=4','Interpreter',...
    'latex','FontSize',legfont)
