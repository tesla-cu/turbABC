...
import postprocess.make_pdfs as make_pdfs
from pyabc.kde import find_MAP_kde, kdepy_fftkde, gaussian_kde_scipy
from plotting.plotting import plot_dist_pdf


def output_by_percent(result, dist, C_limits, x_list, num_bin_raw, num_bin_kde, output_folder, mirror=False):
    """ Postprocess results for list of different acceptance rates.
    :param result: np.array of all parameters (shape (N_samples, N_params))
    :param dist: np.array of all distances corresponding to parameters
    :param C_limits: list of bounds for each parameter
    :param x_list: list of acceptance rates
    ...
    # plot_dist_pdf(output_folder, dist, 0.03)
...
def collect_MAP_values(output_folder, x_list, n_bin_smooth):
    ...
    np.savetxt(os.path.join(output_folder, 'MAP_values'), np.array(MAP))
    return
